...
function renderBlock(block) {
  ...
  // ── rect 块：Mermaid 的 `rect rgb(...)` 语法，用于背景色高亮区域 ──
  // 只渲染纯填充背景矩形，不添加边框和 header tab。
  // 注意：SVG 后处理中 cleanHardcodedColors() 会清除 fill="rgb(...)" 属性，
  // 因此使用 CSS fill 规则通过 style 属性写入（同样会被清，但 opacity 仍保留），
  // 并通过 data-bm-color 属性保存原始颜色，由 injectStylesToSVG 阶段的 CSS 注入还原。
  if (block.type === 'rect') {
    const rawColor = block.label ? block.label.trim() : '';
    const isColor = /^(rgb|rgba|hsl|hsla)\s*\(|^#[0-9a-fA-F]{3,8}$/.test(rawColor);
    // data-bm-color 保存原始颜色（后处理阶段会读取并注入 CSS）
    const colorAttr = isColor ? ` data-bm-color="${escapeAttr2(rawColor)}"` : '';
    // opacity 控制透明度：rgba/hsla 已含 alpha，直接全量显示；其他使用 0.18
