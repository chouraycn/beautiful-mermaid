...
> **任务识别规则（强制）**：每次用户请求生成新的流程图/图表时，都是一个**全新的任务**，必须从 skill 起点开始，**不继承任何上一个任务的上下文**。
>
> - **默认行为**：用户没有明确说"继续"、"接着上一个"、"在之前基础上"时，AI **必须**将其视为全新任务
> - **全新任务意味着**：
>   1. 不读取、不参考工作区现有的 .mmd 文件（除非用户明确指定）
>   2. 不继承上一个任务的业务场景（如"免税"）
>   3. **必须**先打开 `preview.html` 让用户确定主题和预设
>   4. 根据用户当前描述的内容生成新的 .mmd 文件
> - **继续任务的唯一条件**：用户明确说"继续"、"接着上一个"、"在之前基础上"、"在XX基础上继续"时，才可在之前的任务上继续
