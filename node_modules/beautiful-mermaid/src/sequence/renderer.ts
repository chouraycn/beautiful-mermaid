...
/**
 * Render a block background (loop/alt/opt/rect).
 * Wrapped in <g class="block"> with semantic data attributes.
 *
 * Special case: `rect` blocks (Mermaid background-highlight syntax) are rendered
 * as a plain semi-transparent fill — no border, no header tab.
 */
function renderBlock(block: PositionedBlock): string {
  ...
  // ── rect 块：Mermaid 的 `rect rgb(...)` 语法，纯背景色高亮区域 ──
  // data-bm-color 保存原始颜色；injectStylesToSVG 的 Step 2.5 会在 cleanHardcodedColors 之后
  // 把颜色值以 style="fill:..." 写回，避免被清理掉。
  if (block.type === 'rect') {
    const rawColor = block.label ? block.label.trim() : ''
    const isColor = /^(rgb|rgba|hsl|hsla)\s*\(|^#[0-9a-fA-F]{3,8}$/.test(rawColor)
    const colorAttr = isColor ? ` data-bm-color="${escapeAttr(rawColor)}"` : ''
    const opacity = isColor && /^(rgba|hsla)\s*\(/.test(rawColor) ? '1' : '0.18'
    parts.push(
      `  <rect class="seq-rect-bg"${colorAttr} x="${block.x}" y="${block.y}" width="${block.width}" height="${block.height}" ` +
      `rx="0" ry="0" fill="var(--_group-fill)" opacity="${opacity}" />`
    )
    parts.push('</g>')
    return parts.join('\n')
  }

  // ── 其他块类型（loop / alt / opt / par / critical / break）──
  // Outer rectangle
  parts.push(
