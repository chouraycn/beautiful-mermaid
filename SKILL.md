...
description: 将 Mermaid 图表渲染为美观的 SVG、PNG 或 ASCII 艺术字。支持流程图、序列图、状态图、类图、ER图、XY图表（含柱状图、折线图、组合图等）等6种核心图表类型，提供17+内置主题和自定义主题能力，支持CSS级样式定制和交互式预览。适用于在终端、聊天界面或网页中可视化数据流、系统架构、状态机等。
...
- **17 内置主题**：tokyo-night、dracula、github-dark、nord 等（11 暗色 + 6 亮色）
...
| `orange-dark` | 暗色 | `#1c1410` | `#f97316` | glass | 暖色系深色主题 |
| `orange-light` | 亮色 | `#fffbf5` | `#ea580c` | modern | 暖色系浅色主题 |
...
> **任务识别规则（强制执行）**：每次用户请求生成新的流程图/图表时，都是一个**全新的任务**，必须**完全按照以下流程执行**，不得跳过任何步骤。
>
> ## 新任务的标准流程（必须全部执行）
>
> **Step 1 — 打开预览工具（第一优先级）**
> - **立即执行**：`preview_url("file://" + skill目录 + "assets/preview.html")`
> - 不要问用户任何问题，先打开 preview.html
> - 等用户在预览界面选择主题和预设
>
> **Step 2 — 确认用户选择**
> - 用户选择完主题和预设后，底部会显示 CLI 命令
...
>
> **Step 4 — 执行渲染**
> - 使用 Step 2 记录的主题和预设执行渲染
> - 用 `preview_url` 打开生成的 HTML 结果
>
> ---
>
...
>
> 1. **打开 preview.html**（使用 preview_url 工具）
> 2. **【关键】暂停所有操作**
> 3. **明确告诉用户**："请选择主题和预设，选择完成后告诉我"
> 4. **等待用户确认选择完成后**，才能继续生成 .mmd 文件
>
> **禁止行为**：
...
>   1. **打开 preview.html**（使用 preview_url 工具）
>   2. **【关键】暂停所有操作，等待用户选择主题和预设**
>   3. **【关键】只有用户明确告诉你选择完成后，才能继续下一步**
>   4. 记录选择的主题和预设（如 tokyo-night + glass）
>   5. 生成新的 .mmd 文件
>   6. 渲染并预览结果
>
...
> - **AI 自动检测规则**：优先查找用户级 skill 目录（`~/.workbuddy/skills/beautiful-mermaid/`），若不存在则查找项目级（`项目/.workbuddy/skills/beautiful-mermaid/`），拼接 `assets/preview.html`
>
...
```
preview_url("file://" + skill目录 + "assets/preview.html")
```
...
AI 直接用 `preview_url` 工具打开 `file://` + skill目录 + `assets/preview.html`（无需服务器，见上方"AI 预览工作规则"）。用户在页面中选择主题和预设后，底部会实时显示 CLI 命令栏：
