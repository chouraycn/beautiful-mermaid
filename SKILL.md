...
description: 将 Mermaid 图表渲染为美观的 SVG、PNG 或 ASCII 艺术字。支持流程图、序列图、状态图、类图、ER图、XY图表（含柱状图、折线图、组合图等）等6种核心图表类型，提供15+内置主题和自定义主题能力，支持CSS级样式定制和交互式预览。适用于在终端、聊天界面或网页中可视化数据流、系统架构、状态机等。
...
- **15 内置主题**：tokyo-night、dracula、github-dark、nord 等（10 暗色 + 5 亮色）
...
```bash
# 第1步：查找实际安装目录（目录名取决于 zip 文件名）
find ~/.workbuddy/skills -name "SKILL.md" -exec grep -l "name: beautiful-mermaid" {} \;
# 取结果的父目录为 SKILL_DIR，如：/Users/<user>/.workbuddy/skills/<实际目录名>

# 第2步：用实际路径打开（禁止硬编码 beautiful-mermaid）
preview_url("file://" + SKILL_DIR + "/assets/preview.html")
```

> ⚠️ **重要**：skill 安装目录名 = zip 文件名（不含 .zip），**不一定**是 `beautiful-mermaid`。若用户将 zip 改名，安装目录也会变。**每次使用前必须用 `find` 或 `ls ~/.workbuddy/skills/` 动态检测实际目录名**，不得硬编码。
...
AI 先通过以下命令找到实际安装目录（目录名取决于 zip 文件名）：
```bash
find ~/.workbuddy/skills -name "SKILL.md" -exec grep -l "name: beautiful-mermaid" {} \;
```
再用 `preview_url` 工具打开（无需 HTTP 服务器）：
```
preview_url("file://" + <SKILL_DIR> + "/assets/preview.html")
```
